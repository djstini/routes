...
/**
 * Class to handle the registration of logout.nonagon.dev subdomain.
 */
class Logout_Nonagon_Dev extends Subdomain_Handler {
	/**
	...
	public static function update_records( $values ) {
		...
		$interface = new Logout_Nonagon_Dev( $auth_api_token, $zone_id );

		// update the records.
		if ( isset( $values['ipv4'] ) && ! empty( $values['ipv4'] ) ) {
			$ipv4_update = $interface->update_record( $interface->get_record( 'logout', 'A' ), $values['ipv4'] );
		}
